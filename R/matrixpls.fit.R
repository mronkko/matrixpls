...
#'
#'@param S A numeric matrix or data frame containing the manifest variables.
...
#'(2005) PLS path modeling. \emph{Computational Statistics & S Analysis},
#'\bold{48}, pp. 159-205.
#'
...
#'  }
#'
matrixpls.fit <- function(S, inner_matrix, outer_list, modes = NULL, scheme = "centroid", scaled = TRUE, tol = 1e-05, iter = 100) {
    ...
    res = list(out.weights = W, loadings = W_unscaled, path.coefs = Path, r.sqr = R2, effects = C, outer.cor = IC)
