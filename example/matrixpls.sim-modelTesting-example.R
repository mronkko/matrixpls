...
nobservations=300  # number of observations
multi= FALSE       # parallel computing TRUE or FALSE

# Create data sets
datmat.factor <- sim(nRep = ndraws, model = MODEL, n = nobservations, dataOnly = TRUE, multicore =F)    
...
# save estimates 
plsc.res <- sim.res@extraOut # same as I get 

# Calculate the model implied correlation matrix for every estimation
Sighats <- lapply(plsc.res,function(x) fitted(x))  

# Function that calculates the squared Euclidean distance between the empirical and the model implied correlation matrix for every estimation
...
# Function that tranforms the data sets in the way proposed by Yuan & Hayashi (2003), see also Dijkstra & Henseler (2015)
...
scaleddata <- mapply(scaleDataSet, 
                  data=datmat.factor,
                  Sig=Sighats,
                  SIMPLIFY = FALSE)     # I get the same transformed data sets
...
# The results support the findings by Dijkstra & Henseler (2015) (Table 7), although the results for the model considered are slightly better.
...
#Dijkstra, Theo K., and J?rg Henseler. "Consistent and asymptotically normal PLS estimators for linear structural equations." Computational statistics & data analysis 81 (2015): 10-23.

#Henseler, J?rg, and Sarstedt, Marko. "Goodness-of-fit indices for partial least squares path modeling." Computational Statistics 28.2 (2013): 565-580.
