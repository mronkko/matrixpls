...
#
# Figure 2. Distribution of reliability for partial least squares (PLS) Mode A, PLS Mode B, and summed scales in the two-construct model over 500 replications.
#
# from
# 
...
modeA <- matrixpls.sim(nRep = REPLICATIONS, model = MODEL, n = SAMPLE, 
							# Generate data by first generating exogenous variables from normal distribution
							sequential = TRUE,
							# Control parameters
							boot.R = FALSE, stopOnError = TRUE, fitIndices = NULL)
