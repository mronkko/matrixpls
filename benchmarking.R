...
time1 <- system.time(a <- plspm(satisfaction, sat_inner, sat_outer, sat_mod, scaled=FALSE, boot.val = TRUE, br=1000))
...
time2 <- system.time(b <- matrixpls.plspm(satisfaction, sat_inner, sat_outer, sat_mod, scaled=FALSE, boot.val = TRUE, br=1000))

cores = detectCores()

#
# Apply MatrixPLS, multiple threads
#
...
time3 <- system.time(c <- matrixpls.plspm(satisfaction, sat_inner, sat_outer, sat_mod, scaled=FALSE, boot.val = TRUE, br=1000))

#
# Apply MatrixPLS, multiple threads, native call
#
...
time4 <- system.time(d <- boot(satisfaction[,1:27],
